...
def coll_calc(ref_beams, collimate_beams, res, screen_width, wvl, waist, delz):
    
    if collimate_beams == True:
        free_space_diff = prop.BeamProfile(
            res, screen_width, wvl)
        free_space_diff.laguerre_gaussian_beam(0, 0, waist, delz)
    
        for i in range(len(ref_beams)):
            ref_beams[i] *= np.exp(1j * np.angle(free_space_diff.field))

    return ref_beams

def hg_decomp_calc(ref_beams, pascals_row, decomp_wst, res, screen_width, wvl, mode_num):
...
def hg_reconstruction(hg_decomp, ref_beams, pascals_row, decomp_wst, res, screen_width, mode_num):
    ...
    for i in range(mode_num):
        recon_beam += hg_modes[i] * hg_decomp[0][i]
    
    fig, axs = plt.subplots(1, 2)
    axs[0].imshow(np.abs(recon_beam))
    axs[1].imshow(np.abs(ref_beams[0]))
    plt.show()
    plt.close()
...
'''
    ...
    ref_beams = coll_calc(ref_beams, collimate_beams, res, screen_width, ref_wvl, waist, delz)

    
    hg_decomp_1550 = hg_decomp_calc(ref_beams, 5, 0.024, res, screen_width, ref_wvl, mode_num)
    ...
    for ii, wvl in enumerate(wavelength_lst):
        ...
        beams_wvl = coll_calc(beams_wvl, collimate_beams, res,
    ...
    for i in range(len(wavelength_lst)):
        ...
        hg_decomp_new = hg_decomp_calc(tst, 5, 0.024, res, screen_width, ref_wvl, mode_num)
        ...
        #CROSSTALK POWER
        olap = np.asarray(hg_crsstalk_matrix(hg_decomp_1550, hg_decomp_new))
